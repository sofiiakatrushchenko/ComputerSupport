...
#Question 1 — Which animals have exactly 2 legs?

```{r}
library(dplyr)
zoo %>%
  filter(legs == 2) %>%
  select(animal_name, class_type, legs)
```


#Question 2 — Create a new variable indicating if the animal is aquatic

```{r}
zoo %>%
  mutate(is_aquatic = aquatic == 1) %>%
  select(animal_name, is_aquatic) %>%
  head(10)
```

#Question 3  — Which animals have zero legs?
...
#Question 4 — Average number of legs for animals with and without wings

```{r}
library(dplyr)
zoo %>%
  group_by (legs > 0 & feathers == 1) %>%
  summarise (avg_legs = mean(legs))

```

#Question 5 — Average number of legs by milk production
...
welcome_user <- function(name) {
  paste("Welcome to the Zoo Animal Analysis Universe,", name, "!")
}
...
```{r}
query_animals <- function(data, legs_number) {
  data %>%
    filter(legs == legs_number) %>%
    select(animal_name, class_type, legs)
}
```
...
generate_report <- function(data) {
  ...
  # Average legs by wings
  cat("Average legs for animals with/without wings:\n")
  print(
    data %>%
      mutate(has_wings = feathers == 1) %>%
      group_by(has_wings) %>%
      summarise(avg_legs = mean(legs))
  )
  
  # Average legs by milk production
  cat("\nAverage legs by milk production:\n")
  print(
    data %>%
      group_by(milk) %>%
